...
Index.prototype.parseFilePath = function(path) {
  ...
  var pathHasSubFolders = matches[0] !== matches[2];
  ...
  if(baseDir) {
    type = this.getTypeFromFolder(pathHasSubFolders ? matches[3] : matches[4]);
  }
  ...
  // aura / lwc component handling
  // resolves the bundles files into their actual bundle directory.
  // The full bundle must always be deployed, not just individual files.
  if (pathHasSubFolders && !type.subFolders) {
    folder    = matches[3];
    name      = matches[4];
    fileName  = name + '/' + matches[5];
    subFolder = void 0;
    srcPath   = parentDir + folder + '/' + name;
  }
